...
struct PremiumWhatsNewView: View {
    ...
    private var groups: [Group] {
        [Group(emoji: "✅", name: "Implemented & connected", tint: StrandPalette.recoveryColor(80), items: [
            ...
            Item(screen: "Global", component: "Count-up + draw-in animation", change: "Home/Sleep/Trends/Heart hero numbers now use the shipping CountUpText + StrandMotion.drawIn primitives (the same ones RecoveryRing already used) — real ticking numbers and ring draw-in, not static text.", status: .connected),
        ]),
        Group(emoji: "⚠️", name: "Known limitations", tint: StrandPalette.gold, items: [
            Item(screen: "Global", component: "Ripple + section-stagger parity", change: "The HTML mock's tap-ripple and per-section staggered reveal aren't reproduced on every Premium screen yet — count-up/draw-in (the more prominent animation ask) is done; ripple/stagger is a smaller follow-up.", status: .limitation),
